...
exports.default = function (state) {
  ...
  return new Promise(function (resolve, reject) {
	...
	terms.map(function (term, index) {
		...
		if (!indexdeezer && term.toLowerCase() === 'titre') indexdeezer = true;	
		if (!indexdeezer && term.toLowerCase() === 'artiste') indexdeezer = true;
		if (!indexdeezer && term.toLowerCase() === 'album') indexdeezer = true;
	...
	if (sentence) {
		...
		_wtf_wikipedia2.default.from_api(sentence, 'fr', function (response) {
			var deezeralbum = _wtf_wikipedia2.default.plaintext(response);
			// Filtre sur les caractères indésirables
			deezeralbum = deezeralbum.replace(/\[/g, "")
						.replace(/\]/g, "")
						.replace(/\{/g, "")
						.replace(/\}/g, "")
						.replace(/\' /g, " ")
